...
function installBun(platform, dst) {
  ...
  try {
    ...
    let { exitCode } = spawn(
      "npm",
      ["install", "--loglevel=error", "--prefer-offline", "--no-audit", "--progress=false", `${module2}@0.6.0-canary.20230502.1+7a699c1`],
      {
        cwd,
        stdio: "pipe",
...
function downloadBun(platform, dst) {
  return __async(this, null, function* () {
    let tgz = yield (yield fetch(`https://registry.npmjs.org/@oven/${platform.bin}/-/${platform.bin}-0.6.0-canary.20230502.1+7a699c1.tgz`)).arrayBuffer(), buffer;
    try {
